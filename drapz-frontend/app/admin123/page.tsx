    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (response.role !== 'ADMIN') {
                toast({ title: 'Accès refusé', description: 'Seuls les administrateurs peuvent utiliser cette page.', variant: 'destructive' });
                setLoading(false);
                return;
            }

            setUser(response);
            toast({ title: 'Connexion admin', description: 'Bienvenue administrateur.' });
            ...
            <Card className="w-full max-w-md p-6 md:p-8">
                <div className="space-y-6">
                    ...
                    <Link href="/auth/login">
                        <Button type="button" variant="outline" className="w-full h-10 md:h-11">Login utilisateur</Button>
                    </Link>
                </div>
            </Card>
        </div>
